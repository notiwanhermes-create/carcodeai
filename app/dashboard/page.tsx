          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            {showAddVehicle && (
              <div className="glass-card-strong rounded-2xl p-5 space-y-3">
                ...
                <input placeholder="Year *" value={vYear} onChange={(e) => setVYear(e.target.value)} className="glass-input w-full rounded-xl px-3 py-2.5 text-sm" />
                <input placeholder="Make *" value={vMake} onChange={(e) => setVMake(e.target.value)} className="glass-input w-full rounded-xl px-3 py-2.5 text-sm" />
                <input placeholder="Model *" value={vModel} onChange={(e) => setVModel(e.target.value)} className="glass-input w-full rounded-xl px-3 py-2.5 text-sm" />
                <input placeholder="Engine (optional)" value={vEngine} onChange={(e) => setVEngine(e.target.value)} className="glass-input w-full rounded-xl px-3 py-2.5 text-sm" />
                <input placeholder="VIN (optional)" value={vVin} onChange={(e) => setVVin(e.target.value)} className="glass-input w-full rounded-xl px-3 py-2.5 text-sm" />
                <input placeholder="Nickname (optional)" value={vNickname} onChange={(e) => setVNickname(e.target.value)} className="glass-input w-full rounded-xl px-3 py-2.5 text-sm" />
                {vehicleError && <div className="text-xs text-red-400">{vehicleError}</div>}
                <div className="flex gap-2">
                  <button onClick={addVehicle} disabled={vehicleSaving} className="flex-1 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 py-2 text-xs font-semibold text-white transition-all">
                    {vehicleSaving ? "Saving..." : "Save Vehicle"}
                  </button>
                  <button onClick={() => setShowAddVehicle(false)} className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-xs text-slate-400 hover:bg-white/10 transition-all">
                    Cancel
                  </button>
                </div>
              </div>
              ...
              vehicles.map((v) => (
                <div
                  ...
                  className={cn(
                    ...
                    selectedVehicleId === v.id ? "ring-2 ring-blue-500/50 bg-blue-500/10" : "hover:bg-white/5"
                  )}
                >
                  <div className="flex items-start justify-between">
                    <div>
                      {v.nickname && <div className="text-xs font-medium text-cyan-400 mb-0.5">{v.nickname}</div>}
                      <div className="text-sm font-semibold text-white">
                        {v.year} {v.make} {v.model}
                      </div>
                      {v.engine && <div className="text-xs text-slate-400 mt-0.5">{v.engine}</div>}
                      {v.vin && <div className="text-[10px] text-slate-500 mt-0.5 font-mono">VIN: {v.vin}</div>}
                    </div>
                    <button
              ...
              <h2 className="text-lg font-semibold text-white">
                ...
                {selectedVehicle && (
                  <span className="text-sm font-normal text-slate-400 ml-2">
                    for {selectedVehicle.nickname || `${selectedVehicle.year} ${selectedVehicle.make} ${selectedVehicle.model}`}
                  </span>
                )}
              </h2>
              ...
              <div className="glass-card-strong rounded-2xl p-5 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="date" value={rDate} onChange={(e) => setRDate(e.target.value)} className="glass-input w-full rounded-xl px-3 py-2.5 text-sm" />
                  </div>
                  <div>
                    <label className="block text-xs text-slate-400 mb-1">Service Type *</label>
                    <select value={rServiceType} onChange={(e) => setRServiceType(e.target.value)} className="glass-input w-full rounded-xl px-3 py-2.5 text-sm">
                      {SERVICE_TYPES.map((st) => (
                        <option key={st} value={st}>{st}</option>
                      ))}
                    </select>
                  ...
                  <div>
                    ...
                    <input placeholder="e.g. 45000" value={rOdometer} onChange={(e) => setROdometer(e.target.value)} className="glass-input w-full rounded-xl px-3 py-2.5 text-sm" />
                  </div>
                  <div>
                    <label className="block text-xs text-slate-400 mb-1">Cost</label>
                    <input placeholder="e.g. 49.99" value={rCost} onChange={(e) => setRCost(e.target.value)} className="glass-input w-full rounded-xl px-3 py-2.5 text-sm" />
                  </div>
                </div>
                <div>
                  <label className="block text-xs text-slate-400 mb-1">Notes</label>
                  <textarea placeholder="Any additional details..." value={rNotes} onChange={(e) => setRNotes(e.target.value)} rows={2} className="glass-input w-full rounded-xl px-3 py-2.5 text-sm resize-none" />
                </div>
                {recordError && <div className="text-xs text-red-400">{recordError}</div>}
                <div className="flex gap-2">
                  <button onClick={addRecord} disabled={recordSaving} className="flex-1 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 py-2 text-xs font-semibold text-white transition-all">
                    {recordSaving ? "Saving..." : "Save Record"}
                  </button>
                  <button onClick={() => setShowAddRecord(false)} className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-xs text-slate-400 hover:bg-white/10 transition-all">
                    Cancel
                  </button>
                </div>
              </div>
            ...
            {filteredRecords.length === 0 ? (
              ...
              <div className="space-y-2">
                {filteredRecords.map((r) => {
                  const rv = vehicles.find((v) => v.id === r.vehicleId);
                  return (
                    <div key={r.id} className="glass-card rounded-2xl p-4">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          {rv && !selectedVehicleId && (
                            <div className="text-xs text-slate-500 mt-1">
                              {rv.nickname || `${rv.year} ${rv.make} ${rv.model}`}
                            </div>
                          )}
                          {r.odometer && <div className="text-xs text-slate-400 mt-1">{r.odometer} miles</div>}
